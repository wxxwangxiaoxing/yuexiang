...
## 当前代码可直接联调的最小示例（Phase 1）

> 本节以当前仓库里的实际实现为准，可直接用于本地联调。
> 后文的大量时序图、卡片流和 function calling 设计仍然保留，但**当前代码尚未全部实现**。
...
- 当前 SSE 事件类型仅有：`session`、`delta`、`done`、`error`
- 当前消息已支持基础持久化字段：`message_type`、`cards_data`、`tool_name`、`tool_args`、`tool_result`、`finish_reason`、`token_usage`、`error_code`
- 但当前真实对话主链路仍以文本回复为主，`cards_data` / `tool_*` 字段暂未完全进入业务闭环

### 联调前提

```plain
1. 先执行 docs/sql/ai_phase1_migration.sql
2. 使用 JDK 17 启动服务
3. 准备合法登录态 Token
...
```bash
curl -X GET "http://localhost:8080/api/ai/chat/hot-questions"
```
...
```json
{
  ...
  "msg": "操作成功",
  "data": [
    {"id": 1, "text": "附近有啥好吃的", "icon": "food"},
    {"id": 2, "text": "人均50聚餐推荐", "icon": "budget"},
    {"id": 3, "text": "安静约会餐厅", "icon": "date"}
  ]
}
```
...
```bash
curl -X POST "http://localhost:8080/api/ai/chat/session" \
...
```json
{
  ...
  "msg": "操作成功",
  "data": {
    "sessionId": "8f0b8c0f0a4b45b8a5a94c2f0e4d8a77",
    "title": "新的探店会话",
    "messageCount": 0,
    "status": 1,
    "lastActiveTime": 1775831400000,
    "totalTokens": 0,
    "summary": null,
    "contextJson": "{}",
    "longitude": 116.4812,
...
```json
{
  ...
  "data": {
    ...
    "messageCount": 2,
    "status": 1,
    "lastActiveTime": 1775831460000,
    "totalTokens": 0,
    "summary": "附近有啥好吃的？",
    "contextJson": "{\"lastUserMessage\":\"附近有啥好吃的？\",\"lastAssistantMessage\":\"...\",\"recommendedShopIds\":[101,102,103],\"messageCount\":0}",
    "longitude": 116.4812,
    "latitude": 39.9961,
    "shops": [
      {
        ...
        "distance": 320.0,
        "address": "示例地址A",
        "reason": "评分 4.8，320 米内可达，人均 68 元"
      }
    ],
    "messages": [
      {
        ...
        "errorCode": null,
        "shopIds": "[]",
        "createTime": 1775831460000
      },
      {
        "messageId": 2,
        "role": "assistant",
        "content": "我根据你当前位置附近的真实商户数据，先给你推荐这 3 家店：...",
        "messageType": "text",
        "cardsData": null,
        "toolName": null,
        "toolArgs": null,
...
```bash
curl -N -X POST "http://localhost:8080/api/ai/chat/send/stream" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "sessionId": "8f0b8c0f0a4b45b8a5a94c2f0e4d8a77",
    "question": "想找一家适合约会的餐厅",
    "longitude": 116.4812,
    "latitude": 39.9961
  }'
```
...
```text
event: session
data: {"sessionId":"8f0b8c0f0a4b45b8a5a94c2f0e4d8a77","title":"附近有啥好吃的？"}

event: delta
data: {"content":"我根据你当前位置附近的真实商户数据，先给你推荐这 "}
...
```bash
curl -X GET "http://localhost:8080/api/ai/chat/history/8f0b8c0f0a4b45b8a5a94c2f0e4d8a77" \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X GET "http://localhost:8080/api/ai/chat/sessions?pageNo=1&pageSize=10" \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X DELETE "http://localhost:8080/api/ai/chat/session/8f0b8c0f0a4b45b8a5a94c2f0e4d8a77" \
  -H "Authorization: Bearer <token>"
```
...
| 旧接口 | 当前行为 |
| --- | --- |
| `POST /api/ai/chat` | 内部转到非流式发送逻辑 |
| `POST /api/ai/chat/stream` | 内部转到 SSE 发送逻辑 |
| `GET /api/ai/session/{sessionId}` | 内部转到会话详情查询 |
| `DELETE /api/ai/session/{sessionId}` | 内部转到会话删除 |

### 9）推荐联调顺序

```plain
1. GET  /api/ai/chat/hot-questions
2. POST /api/ai/chat/session
3. POST /api/ai/chat/send
4. GET  /api/ai/chat/history/{sessionId}
5. GET  /api/ai/chat/sessions
6. POST /api/ai/chat/send/stream
7. DELETE /api/ai/chat/session/{sessionId}
```
